...
def gemini(prompt: str, uid: str) -> str:
    ...
    print("Gemini raw:", txt)          # shows in Render logs
    return txt
...
def format_body(txt: str) -> str:
    ...
    for ln in txt.splitlines():
        s = ln.strip()
        if s.startswith("•"): continue          # bullets → buttons only
        if s.lower().startswith(("from:","subject:")):
            s = f":email: *{s}*"
        out.append(s)
    return "\n".join(out).strip()

bullet = re.compile(r"^(?:•|\*|[A-Da-d]\)|\d+\))\s*(.+)$")
...
def blocks(raw: str, score:int):
    ...
    opts = choices(raw)
    return [
        {"type":"section","text":{"type":"mrkdwn","text":body or "_(no text)_" }},
        {"type":"context","elements":[{"type":"mrkdwn","text":f"*Score:* {score}"}]},
        {"type":"actions","elements":[
            {"type":"button","text":{"type":"plain_text","text":o},
             "value":o,"action_id":f"choice_{i}"} for i,o in enumerate(opts)
        ]}
    ]
...
def slash():
    ...
    if txt in {"","menu","start"}:
        ...
        return jsonify({
            ...
            "text":"fallback",               # Slack fallback text
            "replace_original":True,         # ← important
            "blocks":[{"type":"section",
                       "text":{"type":"mrkdwn",
...
def interactive():
    ...
    get_chat(uid)     # make sure session exists

    if choice=="start":
        chat_sessions[uid]["score"]=0
        raw = gemini("start training",uid)
    elif choice=="help":
        raw = ("Everything you do is private (ephemeral). "
               "Pick a button; good choices raise your score.")
    elif choice=="exit":
    ...
    return jsonify({
        "response_type":"ephemeral",
        "text":"fallback",
        "replace_original":True,        # replace the message that had the button
